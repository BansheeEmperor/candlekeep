...
### Prismatic Dispersal (Retired)
**Technical Name:** Sine-Distance Diversity Reranking
**Status:** Implemented, benchmarked, and removed. The sine math was sub-millisecond, but the technique required re-embedding expanded candidate texts at query time (~160ms CPU). Comprehensive benchmarking showed <1% MRR improvement across all configurations. See [DESIGN.md § 3.8](DESIGN.md#38-prismatic-dispersal-retired) and Research Diary Entries 43–44 for the full analysis.
...
### The Rosetta Seal
**Technical Name:** Corpus-Derived BM25 Token Normalisation
**The Lore:** An ancient seal inscribed upon the library's index that translates between surface forms of the same word. When a scholar writes `crossencoder` but the scrolls say `cross-encoder`, the Rosetta Seal recognises them as one and the same, ensuring the lexical search finds its mark. The seal is derived automatically from the corpus itself — no scribe need curate it. It is reforged whenever the library is repopulated, adapting to whatever surface-form variations the new scrolls contain. Operators may inspect the seal's mappings in `normalisation_map.json` to understand what translations are being applied. See [ARCHITECTURE.md](ARCHITECTURE.md#the-rosetta-seal-normalisation-map) for configuration and [Research Diary Entry 55](RESEARCH_DIARY.md#entry-55-the-rosetta-seal--corpus-derived-bm25-token-normalisation) for the full analysis.
