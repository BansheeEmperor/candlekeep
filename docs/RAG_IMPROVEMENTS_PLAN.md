...
### LLM-Generated Chunk Summaries (Contextual Retrieval)
Per-chunk context summaries generated by an LLM at ingestion time, prepended to each chunk before embedding. This is a richer version of Bardic Knowledge — instead of document-level title/description, each chunk gets a tailored context sentence explaining its role within the document. The tradeoff is ingestion cost: one LLM call per chunk (~2,770 calls at current corpus scale). Evaluate if Bardic Knowledge's document-level context proves insufficient for specialized corpora where chunks lose meaning without fine-grained context.
